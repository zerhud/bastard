...
constexpr auto mk_te_callable(const factory& f, auto&& src, auto&&... args) {
	...
	constexpr bool call_method_exists = requires{src.orig_val().call(std::declval<data_type>());};
	if constexpr (!is_callable && !call_method_exists) return std::move(src);
	else {
		struct te : src_type, te_callable {
			...
			constexpr data_type call(data_type params) override {
				if constexpr (call_method_exists) return data_type{this->orig_val().call(std::move(params))};
				else return data_type{this->call_val().call(std::move(params))};
			}
		};
